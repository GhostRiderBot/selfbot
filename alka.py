...
#===========Hiburan============#
                        ...
                        elif msg.text.lower().startswith("musik "):
                                sep = msg.text.split(" ")
                                cari = msg.text.replace(sep[0] + " ","")
                                if "|" in text:
                                	alinbdx = cari.split("|")
                                	alinbdx = alinbdx[0]
                                else:
                                	alinbdx = cari
                                try:
                                	r = requests.get("https://rest.farzain.com/api/soundcloud.php?id={}&apikey=PQORJAaWjJT4080oumQiOeqU3".format(str(alinbdx)))
                                	data = json.loads(r.text)
                                	kata = "??RAD MUSIC PLAYLIST?? \n\n"
                                	num = 1
                                	data = data["result"]
                                	for x in data:
                                	    carix = x
                                	...
                                	if "|" in text:
                                	    ...
                                	    uri = x["url_download"]
                                	    imgurl = x["img"]
                                	    cl.sendImageWithURL(to,imgurl)
                                	    kata = "===[%s]===\nurl: %s\nurl download: %s" % (judul,url,uri)
                                	    #line.sendMessage(to,kata)
                                	    cl.sendAudioWithURL(to,uri)
                                	else:
                                	    kata += "\nUntuk mendengarkan lagunya ketik:\nMusik %s|no" % (cari)
                                	    cl.sendMessage(to,kata)
                                except:
                                	logError(error)
